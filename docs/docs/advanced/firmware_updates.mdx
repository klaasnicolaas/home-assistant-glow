...
:::note
Since ESPHome 2024.6.x, the update component can directly retrieve firmware updates from the [glow-energy.io](https://glow-energy.io) manifest. Which has made updating even easier and eliminates the need to use ESPHome Builder (add-on) just for updates.
:::
...
If you are using the ESPHome Builder add-on in Home Assistant, you can also update the firmware through the dashboard. This method is useful if you have adopted the firmware to your ESPHome Builder and know how to tweak the YAML configuration.
...
1. Open the ESPHome Builder (add-on).
...
<p align="left">
  <img 
    ...
    alt="Update firmware through ESPHome Builder (add-on)"
  />
</p>